...
int House::setConnection()
{
	...
	dest_addr.sin_port = htons(atoi(port_number.c_str())); // host byte order
	dest_addr.sin_addr.s_addr = inet_addr(ip_addr.c_str());	// network byte order

	if (-1 == connect(sockfd, (const struct sockaddr *) &dest_addr,
